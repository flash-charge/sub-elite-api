...
export function json(payload, status = 200, methods = 'GET,POST,OPTIONS', request) {
...
export function options(methods = 'GET,POST,OPTIONS', request) {
  return new Response(null, {
    status: isAllowedOrigin(request) ? 204 : 403,
    headers: corsHeaders(methods, request),
  })
}

export function corsHeaders(methods = 'GET,POST,OPTIONS', request) {
  return {
    'access-control-allow-origin': allowedCorsOrigin(request) || DEFAULT_ALLOWED_ORIGIN,
...
export function subscriptionHeaders(request) {
  return {
